...
class LearningSwitch(api.Entity):
    ...
    def handle_link_down(self, port):
        ...
        """
        for p in self.dest_dict.keys():
            if self.dest_dict[p] == port:
                del self.dest_dict[p]
                break
    def handle_rx(self, packet, in_port):
        ...
        if isinstance(packet, basics.HostDiscoveryPacket):
            ...
            self.dest_dict[packet.src] = in_port
            return

        # Flood out all ports except the input port
        self.dest_dict[packet.src] = in_port
        if packet.dst in self.dest_dict.keys():
            self.send(packet, self.dest_dict[packet.dst], flood = False)
        else:
